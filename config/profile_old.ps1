...
$systemS = & "$env:ConEmuDir\system\msys2\usr\bin\cat" "$env:ConEmuDir\config\systems.json" | & "$env:ConEmuDir\bin\jq.exe" -r ".systems[].binpath"
foreach ($system in $systemS){
	$system = $system -replace "`t|`n|`r",""
	$system = $system -replace " ;|; ",";"
	$path = "$env:ConEmuDir\system\$system"
...
$ConEmuModulePath = Join-path $env:ConEmuDir "\system\psmodules\"
...
try {
	...
	# add binaries
	$env:Path += ";$env:ConEmuDir\system\msys2\usr\bin"
}
...
try {
	...
	# mingw
	if ( (Test-Path -Path (Join-Path $env:ConEmuDir "\system\msys2\mingw32\bin")) -and ( $env:PROCESSOR_ARCHITECTURE -eq "x86" ) ) {
		if( -not $env:Path.Contains("$env:ConEmuDir\system\msys2\mingw32\bin") ){
			$env:Path += ";$env:ConEmuDir\system\msys2\mingw32\bin"
		}
	} elseif ( (Test-Path -Path (Join-Path $env:ConEmuDir "\system\msys2\mingw64\bin")) -and ( $env:PROCESSOR_ARCHITECTURE -eq "AMD64" ) ) {
		if( -not $env:Path.Contains("$env:ConEmuDir\system\msys2\mingw64\bin") ){
			$env:Path += ";$env:ConEmuDir\system\msys2\mingw64\bin"
		}
	}
} catch {
...
Set-Alias vi vim
function vim { & ( (Join-Path $env:ConEmuDir '\system\vim\vim74\vim.exe') ) -u (Join-Path $env:ConEmuDir '/config/.vimrc' ) $args  }
...
function bashcall {
	...
	#echo $myinvocation.invocationname
	& ( $env:ConEmuDir + "\system\msys2\usr\bin\bash.exe" ) -c '"' $myinvocation.invocationname $args '"'
}
